...
app.get("/",function(req,res){
  // console.log(posts[0].title+" "+posts[0].post);
...
app.get("/posts/:postname",function(req,res){
  ...
  posts.forEach(function(ele){
    ...
    if(storedTitle===reqTitle){
        res.render("another",{t:ele.title,p:ele.post});
    }
  })
})
app.post("/compose",function(req,res){
  const t = req.body.titleContent;
  const p = req.body.postContent;
  // contentArray.push(t,p);
  let obj ={
    title:t,
    post:p
  }
